...
class FootballMatch extends Component {
  ...
  getValuePlayerType(playertypedetail) {
    ...
    let playertype = 8;
        ...
        if (playertypedetail==="goalkeeper" ) {
           playertype = 8;//Goal Keeper
        } else if (playertypedetail==="defender") {
           playertype = 9;//Defender
        }
        else if (playertypedetail==="midfielder") {
           playertype = 10;//Mid Fielder
        }
        else if (playertypedetail==="striker") {
          playertype = 11;//Forward
       }
        return playertype;
  }
  ...
  converPlayerType(platertype){    
    if(platertype)
    {
      ...
      else if(platertype===9){
        return "defender";

      }else if(platertype===10){
          return "midfielder";
      }
      else if(platertype===11){
        return "striker";
      }
  ...
  toggleAccordionNew(matchDetails) {
    ...
    {        
        ...
        let matchurl="";
          ...
          matchurl=CONST.BACKEND_URL + "/ucmatch/2/" + matchid;
        axios.get(matchurl, config).then(response => {
        formthis.setState({loading:false});
          if (response.data) {
            
            let teamAId = response.data.match.match.home;
            let teamBId = response.data.match.match.away;

                let teamA = response.data.match.teams[teamAId]["players"];
                let teamB = response.data.match.teams[teamBId]["players"];
                
                let teamC = teamA.concat(teamB);
                let seriesid=response.data.match.match.tournament.key;
                let seriesname=response.data.match.match.tournament.name;
                
                formthis.getplayerdetails(matchid, teamC, formthis.state.gameid,seriesid);
                formthis.setState({
                  allPlayerList: response.data.match.players,
                  playerList1: response.data.match.teams[teamAId],
                  playerList2: response.data.match.teams[teamBId],
                  openAccordianID: matchDetails.unique_id,
                  teamname1: response.data.match.teams[teamAId].name,
                  teamname2: response.data.match.teams[teamBId].name,
                  selectedMatchDetails: matchDetails,
                  status_overview:response.data.match.match.status_overview,
                  seriesid:seriesid,
  ...
  getplayerdetails(matchid, players, gameid,seriesid) {
    //  public/api/matchplayerdetails
    //matchid, players[], gameid
  ...
  submitNewAddMatch(matchdetails) {
    ...
    if (this.state.playerList1 && this.state.playerList2) {
      this.state.playerList1.players.forEach((result1, index1) => {
        let objPlayerList = {};
        ...
        objPlayerList["teamname"] = this.state.playerList1.code;
        ...
        objPlayerList["fullname"] = this.state.allPlayerList[result1].legal_name;
        objPlayerList["pname"] = this.state.allPlayerList[result1].name;
        objPlayerList["country"] =(this.state.allPlayerList[result1].nationality && this.state.allPlayerList[result1].nationality.name)?this.state.allPlayerList[result1].nationality.name:this.state.playerList1.code;
      ...
      this.state.playerList2.players.forEach((result2, index1) => {
        let objPlayerList = {};
        ...
        objPlayerList["teamname"] = this.state.playerList2.code;
        ...
        objPlayerList["fullname"] = this.state.allPlayerList[result2].legal_name;
        objPlayerList["pname"] = this.state.allPlayerList[result2].name;
        objPlayerList["country"] =(this.state.allPlayerList[result2].nationality && this.state.allPlayerList[result2].nationality.name)?this.state.allPlayerList[result2].nationality.name:this.state.playerList2.code;
        objPlayerList["isplaying"] = -1;
        playerslist.push(objPlayerList);
      })
    }
    ...
    if (this.state.selectedMatchDetails) {
      ...
      data["team1"] = this.state.playerList1.code;
      data["team2"] = this.state.playerList2.code;
                                ...
                                <Row>
                                  <Col xs="6">
                                    ...
                                    <ListGroup className="forscroll ">
                                      ...
                                      {(this.state.playerList1 && this.state.playerList1.players) ? (this.state.playerList1.players).map((player, index) => (
                                        <ListGroupItem key={index} >
                                          <Row>
                                            ...
                                            <Col xs="4">{formthis.state.allPlayerList[player].legal_name}</Col>
                                            <Col xs="6">
                                              <Row>
                                                <Col xs="6">
                                  ...
                                  <Col xs="6">
                                    ...
                                    <ListGroup className="forscroll">
                                      ...
                                      {(this.state.playerList2 && this.state.playerList2.players) ? (this.state.playerList2.players).map((player, index) => (
                                        <ListGroupItem key={index}>
                                          <Row>
                                            <Col xs="2">
...
export default connect(mapStateToProps)(FootballMatch);
