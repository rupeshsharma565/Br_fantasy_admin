...
class AllUsers extends Component {
  ...
  componentWillReceiveProps(nextProps) {
        //logintype
    if (this.state.isCSV) {
        let newArray=nextProps.users.items?nextProps.users.items.map(({id,username,...element})=>({...element,logindate:moment(element.logindate*1000).format("DD-MM-YYYY hh:mm a"),
       created:moment(element.created*1000).format("DD-MM-YYYY hh:mm a"),
       status:element.status==='0'?'Inactive':'Active',
       isuserverify:(element.isemailverify==='1' && element.isbankdverify==='1' && element.isphoneverify==='1' && element.ispanverify==='1' )?'Yes':'NO',
