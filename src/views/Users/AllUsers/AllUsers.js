...
class AllUsers extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    //logintype
    if (this.state.isCSV) {
      let newArray = nextProps.users.items
        ? nextProps.users.items.map(({ id, username, ...element }) => ({
          ...element,
          logindate: moment(element.logindate * 1000).format(
            "DD-MM-YYYY hh:mm a"
          ),
          created: moment(element.created * 1000).format(
            "DD-MM-YYYY hh:mm a"
          ),
          status: element.status === "0" ? "Inactive" : "Active",
          isuserverify:
            element.isemailverify === "1" &&
              element.isbankdverify === "1" &&
              element.isphoneverify === "1" &&
              element.ispanverify === "1"
              ? "Yes"
              : "NO",
          ...
          isbankdverify: element.isbankdverify === "0" ? "No" : "Yes",
          ispanverify: element.ispanverify === "0" ? "No" : "Yes",
        }))
        : null;
      this.setState({
    ...
    this.setState({
      totalpage: nextProps.users.prevTotal
        ? nextProps.users.prevTotal
        : nextProps.users.total,
    });
  }
  getBadge = (status) => {
  ...
  handleSelectedPaginate(selectedPage) {
    ...
    this.setState({ selectedPage: selectedPage, isCSV: false });
              ...
              <CardBody>
                    ...
                    {this.state.items
                      ? this.state.items.map((user, index) => (
                        <tr key={user.id}>
                          ...
                          <td>
                            {moment(new Date(parseInt(user.created) * 1000))
                              .utcOffset("+05:30")
                              .format("YYYY-MM-DD")}
                          </td>
                          <td>
                            {parseInt(user.logindate)
                              ? moment(
                                new Date(parseInt(user.logindate) * 1000)
                              )
                                .utcOffset("+05:30")
                                .format("YYYY-MM-DD")
                              : "N/A"}
                          ...
                          <td>
                            <Badge color={this.getBadge(user.isphoneverify)}>
                              {user.isphoneverify === "0"
                                ? "Not Verify"
                                : "Verified"}
                            </Badge>
                          </td>
                          <td>
                            <Badge color={this.getBadge(user.isemailverify)}>
                        ...
                        <td><Badge color={this.getBadge(user.ispanverify)} >{user.ispanverify==="0"?"Not Verify":"Verified"}</Badge></td> */}
                          <td>
                            <Badge color={this.getBadge(user.status)}>
                              {user.status === "1"
                                ? "Active"
                                : user.status === "0"
                                  ? "Inactive"
                          ...
                          <td>
                            {user.status === "0" || user.status === "2" ? (
                              <span>
                                <Button
                                  className="btn-sm btn-square btn-success"
                                  onClick={() =>
                                    this.udpateUserStatus(user, 1)
                                  }
                                >
                                  Active&nbsp;&nbsp;&nbsp;
                                  </Button>{" "}
                            ...
                            {user.status === "1" || user.status === "2" ? (
                              <span>
                                <Button
                                  className="btn-sm btn-square btn-danger"
                                  onClick={() =>
                                    this.udpateUserStatus(user, 0)
                                  }
                                >
                                  Inactive
                                  </Button>
                ...
                {parseInt(this.state.totalpage) > 10 ? (
                  <PaginationComponent
                    ...
                    activePage={parseInt(this.state.selectedPage)}
                  // defaultActivePage={this.state.selectedPage}
                  />
                ) : null}
              </CardBody>
            </Card>
